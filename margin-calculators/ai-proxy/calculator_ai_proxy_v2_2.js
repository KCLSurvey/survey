...
const ALLOWED_ORIGINS = String(process.env.ALLOWED_ORIGIN || 'https://kclsurvey.github.io,http://localhost:3000,http://127.0.0.1:3000').split(',').map(x => x.trim()).filter(Boolean);
const MAX_BODY_BYTES = Number(process.env.MAX_BODY_BYTES || 2000000);
...
function corsHeaders(origin) {
  const allow = ALLOWED_ORIGINS.includes('*') ? '*' : (isAllowedOrigin(origin) ? origin : ALLOWED_ORIGINS[0]);
  return {
...
Предпочтительно верни JSON {"answer":"полный ответ","warnings":[],"actions":[]}.
...
function safeParseResult(text, allowActions) {
  ...
  if (!parsed || typeof parsed !== 'object') {
    return { answer: cleaned, warnings: [], actions: [] };
  }
...
function extractOpenRouterFinalText(data) {
  ...
  const candidates = [
    ...
    contentToText(msg.final),
    contentToText(msg.answer),
...
async function callOpenRouter(question, snapshot, allowActions) {
  ...
  if (!text) {
    console.warn(new Date().toISOString(), 'Empty final content from model; retrying in plain-text final-answer mode', {
  ...
  return safeParseResult(text, allowActions);
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (!isAllowedOrigin(origin)) return sendJson(res, 403, { error: 'Origin is not allowed' }, origin);
  ...
  if (req.method === 'GET' && url.pathname === '/health') {
    ...
    return sendJson(res, 200, { ok: true, proxyVersion: PROXY_VERSION, provider: PROVIDER, model: MODEL, keyConfigured }, origin);
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/assistant') {
    ...
    try {
      ...
      return sendJson(res, 200, await callProvider(question, body.snapshot, body.allowActions !== false), origin);
    } catch (error) {
